...
\item There are also instructions $\isub$ for subtraction; $\isubs$, $\isubc$,
...
\item $\imulj$ is also full multiplication without splitting the
  results. $\ismulj$ is the signed variant of $\imulj$.
...
\item $\ishl\ x\ a\ n$ shifts the source atom $a$ left by $n$ and
  ...
  The number of shifts $n$ is an atom and need not be a constant.
  A non-constant $n$ is still checked for safety and carried by the
  range specification, but the algebraic abstraction drops the
  instruction and warns that it has done so, leaving $x$ unrelated to
  $a$; the same holds of $\ishr$ and $\isar$.
  A constant $n$ ranges from $0$, which copies $a$ unchanged, up to but
  excluding the size of $a$; the same holds of $\ishr$ and $\isar$.
\item $\ishls\ o\ x\ a\ n$ is the same as $\ishls\ x\ a\ n$ except
  that the bits shifted out are stored in $o$.
  Here $n$ must be positive, since $o$ holds the $n$ bits shifted out
  and no variable is zero bits wide; the same holds of $\ishrs$ and
  $\isars$.
...
\item $\ispl\ x_h\ x_l\ a\ n$ splits the source atom $a$ at position
  ...
  $x_l$. $\isspl$ is the signed variant of $\ispl$. It splits the
  signed source atom, stores the signed high bits in $x_h$ and the
  unsigned low bits in $x_l$.
  Here $n$ must lie strictly between $0$ and the size of $a$, since
  either end would leave one destination zero bits wide.
\item $\isplit$ is the same as $\ispl$ except that the high bits and
  the low bits are extended to the size of $a$. $\issplit$ is the
  signed variant of $\isplit$.
  While the low bits are always zero extended, the high bits are sign
  extended if $a$ is signed and otherwise zero extended.
  Since both destinations are as wide as $a$, $n$ may be $0$, which
  puts all of $a$ in $x_h$ and leaves $x_l$ zero, or the size of $a$,
  which puts all of $a$ in $x_l$ and leaves $x_h$ zero.
  The latter needs an unsigned $x_h$: for a signed $a$ it would hold
  the sign of $a$ repeated, which no single instruction produces.
...
\item $\ismttocas\ r$ verifies the specified range predicate and then
  assumes it when verifying algebraic properties.
  A range predicate holds of bit-vectors and so is read modulo $2^w$,
  while an algebraic predicate holds over the integers.
  The two readings agree only where no operation in $r$ wraps, so
  \cryptoline also verifies, for every arithmetic operation that the
  types do not already keep in range, that it agrees with the same
  operation carried out at a width wide enough to hold every value the
  types allow.
  Writing an $\iassert$ and an $\iassume$ by hand instead states the
  predicate twice and verifies neither the agreement of the two
  statements nor that of their readings.
  Bit-wise operations, division, rotation, and right shifts have no
  algebraic reading, and neither has a negated or disjunctive
  predicate; $\ismttocas$ rejects them.
...
\item $\ighost$ can introduce logical variables that must only be used
  in specifications such as $\iassert$, $\iassume$, $\ismttocas$,
  $\icut$, $\iecut$, $\ircut$, and postconditions.
  The predicate in a $\ighost$ instruction is always assumed.
\item $\icall\ p(a_1, a_2, \ldots, a_n)$ invokes a defined
  procedure $p$ with the actual parameters $a_1, a_2, \ldots, a_n$
  (call be value for input parameters).
  The call is replaced by the pre-/post-conditions of $p$ for
  verification.
  Thus $\icall$ is currently incompatible with equivalence checking
  and simulation.
\item $\iinline\ p(a_1, a_2, \ldots, a_n)$ inlines a defined
  procedure $p$ with the actual parameters $a_1, a_2, \ldots, a_n$,
  which replace every occurrence of the corresponding formal
  parameters in the procedureal body.
\item $\iinlinespec\ p(a_1, a_2, \ldots, a_n)$ is semantically the
  same as $\iinline$ but $\iinlinespec$ is replaced by the
  pre-/post-conditions of $p$ for verification.
\item $\icase\ n = a\ [k_1, \ldots, k_m]\ \{\ P\ \}$ runs the body $P$
  once for each value $k_i$, with the name $n$ bound to $k_i$ as a
  named integer, and keeps the run whose $k_i$ is the value the atom
  $a$ actually has.
  It is an abbreviation, expanding into $\iseteq$ instructions
  comparing $a$ against each $k_i$, one copy of $P$ per value, and a
  chain of $\icmov$ merging the copies.
  Every variable $P$ assigns is merged; a body that reads a variable it
  also assigns sees the value that variable had before the statement.
  A value may be written as a range, so $[0 .. 3]$ abbreviates
  $[0, 1, 2, 3]$, and the values must be distinct.
  The body refers to the bound value as $\$n$, which lets it stand
  where an instruction requires a constant rather than an atom, such as
  the position of a $\isplit$ or the index of a vector element.
  A $\icut$ may not appear in $P$.
  The statement asserts that $a$ is one of $k_1, \ldots, k_m$, so a
  program that can reach it with any other value fails to verify.
\item $\icase\ n = a\ [k_1, \ldots, k_m]\ \{\ P\ \}\ \ielse\ \{\ Q\ \}$
  runs $Q$ when $a$ is none of $k_1, \ldots, k_m$, and asserts nothing
  about $a$.
  $Q$ is what the merge falls through with, and only what every branch
  assigns, $Q$ included, is an output of the statement.
  The two forms differ in what an algebraic proof can conclude: without
  an $\ielse$ one of the listed values was taken, so $a$ is known to be
  that value, whereas with one the algebra cannot tell which branch ran,
  since whether $a$ fell in the list is a range property.
\item $\irepeat\ n = [k_1, \ldots, k_m]\ \{\ P\ \}$ runs $P$ once for
  each value, in order, with $n$ bound as a named integer.
  The values are not something the program chooses between, so the
  copies are ordinary straight-line code: nothing is merged, and a later
  copy sees what an earlier one assigned.
  A lane of a vector may be assigned by naming it in the destination of
  a $\imov$, as in $\imov\ v[\const]\pmb{,}\ a$, which is what makes
  $\irepeat$ useful for per-lane work.
  The comma is required there, since a bracket after a vector name would
  otherwise begin a vector literal; instruction arguments may be
  separated by commas generally.
\end{itemize}
...
\begin{array}{rclcl}
  \instr &  ::= & \pmb\imov\ \lval\ \atom
         ...
         & \mid & \pmb\isubs\ \lval\ \lval\ \atom\ \atom \\
         ...
         & \mid & \pmb\ismulj\ \lval\ \atom\ \atom
         & \mid & \pmb\inondet\ \lval \\
         & \mid & \pmb\iset\ \lval
         & \mid & \pmb\iclear\ \lval \\
         & \mid & \pmb\ishl\ \lval\ \atom\ \atom
         & \mid & \pmb\ishls\ \lval\ \lval\ \atom\ \const \\
         & \mid & \pmb\ishr\ \lval\ \atom\ \atom
         & \mid & \pmb\ishrs\ \lval\ \lval\ \atom\ \const \\
         & \mid & \pmb\isar\ \lval\ \atom\ \atom
         ...
         & \mid & \pmb\isspl\ \lval\ \lval\ \atom\ \const \\
         & \mid & \pmb\isplit\ \lval\ \lval\ \atom\ \const 
         & \mid & \pmb\issplit\ \lval\ \lval\ \atom\ \const \\
         & \mid & \pmb\ijoin\ \lval\ \atom\ \atom \\
         ...
         & \mid & \pmb\iinline\ \id\ \pmb{(}\ \atom_{\pmb,}^*\ \pmb{)} \\
         & \mid & \pmb\iinlinespec\ \id\ \pmb{(}\ \atom_{\pmb,}^*\ \pmb{)}
         & \mid & \pmb\inop \\
         & \multicolumn{3}{l}{\mid\ \pmb\icase\ \id\ \pmb{=}\ \atom\
           \pmb{[}\ \const_{\pmb,}^+\ \pmb{]}\ \pmb\{\ \instr_{\pmb;}^+\ \pmb\}
           \ [\ \pmb\ielse\ \pmb\{\ \instr_{\pmb;}^+\ \pmb\}\ ]} \\
         & \multicolumn{3}{l}{\mid\ \pmb\irepeat\ \id\ \pmb{=}\
           \pmb{[}\ \const_{\pmb,}^+\ \pmb{]}\ \pmb\{\ \instr_{\pmb;}^+\ \pmb\}} \\
\end{array}
\]
